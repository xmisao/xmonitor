class Xmonitor::Dashboard
  ...
  require "xmonitor/dashboard/server"
  ...
  class Datapoint
    attr_reader :timestamp, :host, :metric, :dimension, :value

    def initialize(*args)
      @timestamp, @host, @metric, @dimention, @value = *args
    end
  end
  ...
  def load_data
    ...
    get_s3_object_and_process_body(output_location) do |body|
      CSV.new(body, headers: true).each{|row|
        data = [Time.parse(row[0]), row[1], row[2], row[3], row[4].to_f]

        data_store[data[1..3]] << Datapoint.new(*data)
      }
    end
